...
class EcommerceScraper {
  ...
  async init() {
    try {
      ...
      try {
        await this.page.overridePermissions('https://www.amazon.com', [
          'geolocation',
          'notifications'
        ]);
      } catch (error) {
  ...
  async navigateToPage(url, retries = 0) {
    try {
      ...
      if (
        ...
        throw new Error(
          'Bot detection triggered - CAPTCHA or robot check detected'
        );
      }
      ...
      if (retries < this.options.maxRetries) {
        console.log(
          `🔄 Retrying... (${retries + 1}/${this.options.maxRetries})`
        );
      ...
      const products = await this.page.evaluate(selectors => {
        ...
        productElements.forEach((element, index) => {
          try {
            ...
            const linkElement =
              element.querySelector(selectors.link) ||
            ...
            const price =
              priceText.replace(/[^\d.,]/g, '').replace(/,/g, '') || '0';
  ...
  async scrapeAmazon(searchTerm) {
    try {
      ...
      if (
        ...
        throw new Error(
          'Amazon has detected automated browsing - please try again later'
        );
      }
      ...
      const selectors = {
        ...
        title:
          'h2 a span, .a-size-mini span, .a-size-base-plus, [data-cy="title-recipe-title"]',
...
Options:
  ...
  console.log(
    `🎯 Starting scraper for ${site} with search term: "${searchTerm}"`
  );
