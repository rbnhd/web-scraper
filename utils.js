...
class ScraperUtils {
  ...
  static logStats(data, startTime) {
    ...
    console.log(
      `   Average time per product: ${(duration / data.length).toFixed(2)} seconds`
    );
  ...
  static validateProduct(product) {
    ...
    const isValid = required.every(
      field =>
        product[field] &&
        product[field] !== 'N/A' &&
        product[field].toString().trim().length > 0
    );

    return {
      isValid,
      issues: required.filter(
        field =>
          !product[field] ||
          product[field] === 'N/A' ||
          product[field].toString().trim().length === 0
  ...
  static generateSummaryReport(data, site, searchTerm, outputPath) {
    ...
    if (data.length > 0) {
      ...
      report.summary = {
        ...
        avgTitleLength:
          data.reduce((acc, item) => acc + item.title.length, 0) / data.length
      };
    }
